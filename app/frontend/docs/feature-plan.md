...
- 리스트 탭: 생성된 시간표 목록(이름, 연도/학기, 삭제 버튼).
- 시간표 상세 탭(한 개 선택 시):
  - 좌측 그리드: 주/요일·시간대 격자에 과목 블록 배치. 블록 색상은 AppTokens.primaryMuted, 충돌 시 붉은 음영.
  - 우측 패널 두 개:
    1) “제외된 과목” 리스트: 대체 시간표 생성 시 제외한 course 목록 표시.
    2) “시간 겹치는 과목” 리스트: 현재 선택된 블록과 시간이 겹치는 과목을 표시(클라이언트에서 classTimes로 계산).
  - 하단 패널 두 개:
    1) “위시리스트에서 추가” 섹션: 위시리스트 그룹(우선순위별)에서 선택 후 추가 버튼.
    2) “강의 목록에서 추가” 섹션: 현재 검색 결과 리스트와 동일 UI, 추가 버튼.
...
4) 시간표 탭: 이름/연도/학기 입력 후 생성 → 목록에서 선택 → 상세 탭에서
   - 위시리스트/강의 목록 패널에서 과목 추가,
