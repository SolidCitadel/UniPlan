...
- **[테스트 격리] docker-compose.test.yml의 Tempo·Loki tmpfs 스토리지**: 테스트 환경의 Tempo와 Loki는 named volume 대신 `type: tmpfs, mode: 0777` 마운트를 사용한다. 컨테이너 재시작 시 데이터가 초기화되어 테스트 실행 간 격리가 보장되며, 이전 실행의 잔류 데이터가 테스트 결과에 영향을 주지 않는다. Grafana와 Promtail positions 파일은 설정 지속성을 위해 named volume을 유지한다.
