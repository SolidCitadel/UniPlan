...
| **API Gateway** | `uniplan-api-gateway` | 8080 | 단일 진입점 (`/api/v1/**`) |
...
> **환경변수 필수**: `.env`가 없으면 `docker compose up` 시 즉시 오류가 발생합니다. `app/frontend/.env.local`이 없으면 `npm run dev` 시 즉시 오류가 발생합니다.

## 3. Observability 스택 (`--profile observability`)

Prometheus + Grafana + Tempo + Loki를 포함한 완전한 Observability 환경을 선택적으로 기동할 수 있습니다.
...
| **Grafana** | 3001 | 메트릭/로그/트레이스 통합 시각화 (admin / GRAFANA_ADMIN_PASSWORD) |
| **Tempo** | 내부망 | 분산 트레이스 수집 (OTLP HTTP 4318, API 3200) |
| **Loki** | 내부망 | 로그 수집/저장 (API 3100) |
| **Promtail** | 내부망 | Docker 컨테이너 로그 수집 → Loki 전송 |
...
Loki에 수집된 구조화 로그를 LogQL로 조회합니다:

1. Grafana 좌측 메뉴 → Explore
2. 데이터소스 드롭다운 → **Loki** 선택
3. LogQL 쿼리 예시:
   ```logql
   # 서비스별 전체 로그
   {service="api-gateway"}
   ...
   # ERROR 레벨 로그만 필터
   {service="planner-service", level="ERROR"}

   # 특정 traceId 로그 (트레이스 → 로그 연계)
   {service=~".+"} | traceId = "abc123..."
   ```
4. 로그 라인 클릭 → **"Tempo에서 트레이스 보기"** 링크 → 연관 트레이스 자동 이동

---

## 4. 테스트 환경 (base + test override)

Integration 테스트를 위해 base 파일과 test override를 함께 사용합니다.

```bash
docker compose -f docker-compose.yml -f docker-compose.test.yml up -d --build
```

## 5. 운영 배포 (base + prod override)

운영 환경에서는 base 파일과 `docker-compose.prod.yml`을 함께 사용하여 시크릿을 주입합니다.
...
> 📝 `docker-compose.prod.yml`은 필요 시 생성해야 합니다.
